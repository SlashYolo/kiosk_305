...
import sys, os, json, time, hashlib, signal, logging, threading, gzip
...
def fetch_and_save(entity_type: str, name: str, session: requests.Session, retries: int = 2) -> str:
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            base = f"{entity_type}_{safe_filename(name)}"
            path_gz   = CACHE_DIR / (base + '.json.gz')
            path_json = CACHE_DIR / (base + '.json')

            # Проверка хэша: пробуем .json.gz, потом .json (обратная совместимость)
            for p, opener in [(path_gz, lambda p: gzip.open(p,'rt',encoding='utf-8')),
                              (path_json, lambda p: open(p,'r',encoding='utf-8'))]:
                if p.exists():
                    try:
                        with opener(p) as f:
                            head = f.read(300)
            ...
            # Хэш не совпал — парсим и сохраняем как .json.gz
            data = r.json()
            with gzip.open(path_gz, 'wt', encoding='utf-8', compresslevel=6) as f:
                f.write(json.dumps({
                    'hash':    raw_hash,
                    'type':    entity_type,
                    'name':    name,
            ...
            # Удаляем старый .json если остался
            if path_json.exists():
                try: path_json.unlink()
                except: pass
            return 'saved'
        except Exception as e:
            if attempt < retries:
